...
// isAmongDefined checks if the current token type is among the expected token types.
//
// Parameters:
// - expectedTokens: A variadic parameter that accepts any number of TokenType values.
//
// Returns:
//   - A boolean value indicating whether the current token type is among the expected token types.
//     Returns true if the current token type is found in the expectedTokens slice, otherwise returns false.
...
// If the read position is at or beyond the end of the input string, it returns 0 (ASCII code for NULL).
//
// Parameters:
// - l: A pointer to the Lexer struct. This function is a method of the Lexer struct, so it has access to its fields and methods.
//
// Returns:
// - A rune representing the next character in the input string. If the read position is at or beyond the end of the input string, it returns 0.
...
// skipWhitespace is a method of the Lexer struct that skips over whitespace characters in the input string.
// It iterates through the characters of the input string, starting from the current character,
// and advances the lexer's position to the next non-whitespace character.
//
// Parameters:
// - l: A pointer to the Lexer struct. This function is a method of the Lexer struct, so it has access to its fields and methods.
//
// Return:
// This function does not return any value. It modifies the lexer's position and current character.
...
// readNumber is a method of the Lexer struct that reads a sequence of digits and an optional decimal point
// from the input string and returns the corresponding number as a string.
//
// The method starts by checking if the current character is a digit. If it is, it appends the digit to the number string
// and advances the lexer's position to the next character. It then checks if the next character is a decimal point.
// If it is, it appends the decimal point to the number string and advances the lexer's position to the next character.
// Finally, it continues reading digits and appending them to the number string until a non-digit character is encountered.
//
// Parameters:
// - l: A pointer to the Lexer struct. This function is a method of the Lexer struct, so it has access to its fields and methods.
//
// Return:
// - A string representing the number read from the input string.
...
func (l *Lexer) ReadString() string {
	var str string
	l.getChar() // Skip the opening quote
	...
	l.getChar() // Skip the closing quote
	return str
}
...
func (l *Lexer) GetNextToken() Token {
	...
	case '0':
		// Special case to handle the end of input
		tok = newToken(EOF, "")
	case ' ', '\t', '\n', '\r':
		// Handling whitespace
	...
	default:
		if IsLetter(string(l.currentChar)) || l.currentChar == '_' {
			...
			switch ident {
			case "let":
				tok = newToken(LET, ident)
			case "const":
				tok = newToken(CONST, ident)
			case "class":
				tok = newToken(CLASS, ident)
			case "new":
				tok = newToken(NEW, ident)
			case "import":
				tok = newToken(IMPORT, ident)
			case "from":
				tok = newToken(FROM, ident)
			case "fn":
				tok = newToken(FN, ident)
			case "if":
				tok = newToken(IF, ident)
			case "else":
				tok = newToken(ELSE, ident)
			case "foreach":
				tok = newToken(FOREACH, ident)
			case "while":
				tok = newToken(WHILE, ident)
			case "for":
				tok = newToken(FOR, ident)
			case "export":
				tok = newToken(EXPORT, ident)
			case "typeof":
				tok = newToken(TYPEOF, ident)
			case "in":
				tok = newToken(IN, ident)
			case "return":
				tok = newToken(RETURN, ident)
			case "break":
				tok = newToken(BREAK, ident)
			case "continue":
				tok = newToken(CONTINUE, ident)
			case "defer":
				tok = newToken(DEFER, ident)
			case "var":
				tok = newToken(VAR, ident)
			case "nil":
				tok = newToken(NIL, ident)
			case "struct":
				tok = newToken(STRUCT, ident)
			case "interface":
				tok = newToken(INTERFACE, ident)
			case "implements":
				tok = newToken(IMPLEMENTS, ident)
			case "extends":
				tok = newToken(EXTENDS, ident)
			case "package":
				tok = newToken(PACKAGE, ident)
			case "function":
				tok = newToken(FUNCTION, ident)
			default:
...
// func (l *Lexer) LookupIdent() TokenType {
// 	switch  {
// 	case "let":
// 		newToken(LET, string(l.currentChar))
// 	case "const":
// 		newToken(CONST, ident)
// 	case "class":
// 		newToken(CLASS, ident)
// 	case "new":
// 		newToken(NEW, ident)
// 	case "import":
// 		newToken(IMPORT, ident)
// 	case "from":
// 		newToken(FROM, ident)
// 	case "fn":
// 		newToken(FN, ident)
...
// 		newToken(ELSE, ident)
// 	case "foreach":
// 		newToken(FOREACH, ident)
// 	case "while":
// 		newToken(WHILE, ident)
// 	case "for":
// 		newToken(FOR, ident)
// 	case "export":
// 		newToken(EXPORT, ident)
// 	case "typeof":
// 		newToken(TYPEOF, ident)
// 	case "in":
// 		newToken(IN, ident)
// 	case "return":
// 		newToken(RETURN, ident)
// 	case "break":
// 		newToken(BREAK, ident)
// 	case "continue":
// 		newToken(CONTINUE, ident)
// 	case "defer":
// 		newToken(DEFER, ident)
// 	case "var":
// 		newToken(VAR, ident)
// 	case "nil":
// 		newToken(NIL, ident)
// 	case "struct":
// 		newToken(STRUCT, ident)
// 	case "interface":
// 		newToken(INTERFACE, ident)
// 	case "implements":
// 		newToken(IMPLEMENTS, ident)
// 	case "extends":
// 		newToken(EXTENDS, ident)
// 	case "package":
// 		newToken(PACKAGE, ident)
// 	case "function":
// 		newToken(FUNCTION, ident)
// 	default:
